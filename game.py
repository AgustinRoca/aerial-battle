...
def main():
    player = 0
    other_player = 1
    agents = [agent1,  player1]
    boards = []
    for agent in agents:
        boards.append(agent.get_starting_board())
    ...
    while has_airships_alive(boards[player], hit_boards[other_player]):
        ...
        try:
            ...
            print(f"Result: {hit_boards[player][coordinates[0]][coordinates[1]][coordinates[2]]}")
        except ValueError:
            print('Repeated shot, MISSED TURN')
        hits = count_total_hits(hit_boards[player])
        if (player == 1 and (i//2 % 20) == 0) or (player == 1 and hits != 0):
